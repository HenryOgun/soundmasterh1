  ...
  return (
    <div className="mt-6 p-5 bg-gray-50 rounded-xl border border-gray-100">
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        {platforms.map(({ label, color, icon, href }) => (
          <a key={label} href={href} target="_blank" rel="noreferrer"
            className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold text-white transition-colors ${color}`}>
            {icon}{label}
          </a>
        ))}
        <button onClick={handleCopyInstagram}
          className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold text-white bg-gradient-to-r from-[#f9ce34] via-[#ee2a7b] to-[#6228d7] hover:opacity-90 transition-opacity">
